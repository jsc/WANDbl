...
#ifndef IDX_INVFILE_HPP
#define IDX_INVFILE_HPP
...
private:
    ...
    sdsl::int_vector<> m_f_t; //yubink: df of document
...
public:
    ...
    result search(const std::vector<query_token>& qry,size_t k,
        ...
        for(const auto& qry_token : qry) {
            pl_data[j++] = plist_wrapper(m_postings_lists[qry_token.token_ids[0]], (double)m_F_t[qry_token.token_ids[0]],(double)qry_token.f_qt);
